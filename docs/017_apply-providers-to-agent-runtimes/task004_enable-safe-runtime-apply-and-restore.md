...
Applying a Provider writes every proposed managed value from the fresh plan, including removing a managed credential field when that Provider has no API key. Applying `Official Default` removes only the managed Codex fields or Claude Code environment entries listed by the plan. For Codex, leave unowned members of the selected configuration Provider table intact and omit the table only when no content remains. This reset is not a snapshot restoration. An Official Default operation against a missing file or a file that already contains none of the managed fields is an external no-op but may still record `official-default` as Foundry's successful state.
...
- [x] Applying a Codex Provider reuses the eligible execution-time custom Provider key or falls back to `foundry_managed`, updates exactly the approved managed fields, and preserves unowned TOML data, comments, order, whitespace, and formatting supported by the patch dependency.
...
- [x] Restore Official Default removes only Foundry-managed fields, preserves unowned fields, does not restore a snapshot, and does not create an empty configuration file when no file or managed fields require a write.
...
- `pnpm test` - Passed: 13 test files and 68 tests.
