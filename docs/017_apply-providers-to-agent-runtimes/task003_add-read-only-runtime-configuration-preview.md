...
For Codex, read `~/.codex/config.toml` with `smol-toml` and resolve the configuration Provider key before constructing a Provider-target field plan. If the current top-level `model_provider` resolves to an explicit table under `model_providers`, reuse that exact key. Otherwise reuse the sole existing Provider table. This preserves the Provider identity stored in existing Codex conversations, such as `custom`, when Foundry first takes over, after restoring Official Default, and across later Foundry Provider switches. Create `foundry_managed` only when no Provider table exists, and reject multiple unselected Provider tables as ambiguous.
